...
def _distributed_train_step(strategy: tf.distribute.Strategy,
  ...
  if 'image_azadNet' in step_outputs.keys():
    image_azadNet = _concat_tensors(step_outputs['image_azadNet'])
    final_output = _concat_tensors(step_outputs['final_output'])
