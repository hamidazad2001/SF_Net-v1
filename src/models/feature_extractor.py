...
def _conv3D(filters: int, name: str):
...
class SubTreeExtractor_3D(tf.keras.layers.Layer):
  ...
  def __init__(self, name: str, config: options.Options):
    ...
    self.convs = []
    for i in range(n):
      self.convs.append(_conv3D(filters=(k << i), name='cfeat_conv3D_{}'.format(2 * i)))
      self.convs.append(_conv3D(filters=(k << i), name='cfeat_conv3D_{}'.format(2 * i + 1)))

  def call(self, image: tf.Tensor, n: int) -> List[tf.Tensor]:
    head = image
    pool = tf.keras.layers.AveragePooling3D(strides=(1,2,2), padding='Same')
    pyramid = []
    for i in range(n):
      head = self.convs[2*i](head)
      head = self.convs[2*i+1](head)
...
class FeatureExtractor_3D(tf.keras.layers.Layer):

  def __init__(self, name: str, config: options.Options):
    super().__init__(name=name)
    self.extract_sublevels = SubTreeExtractor_3D('sub_extractor_3D', config)
    self.options = config
