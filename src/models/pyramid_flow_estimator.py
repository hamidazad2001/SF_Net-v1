...
class FlowEstimator_3D(tf.keras.layers.Layer):

  def __init__(self, name: str, num_convs: int, num_filters: int):
    super(FlowEstimator_3D, self).__init__(name=name)
    def conv(filters, size, name, activation=_relu):
    ...
    self._convs = []
    for i in range(num_convs):
      self._convs.append(conv(filters=num_filters, size=3, name=f'conv3_{i}'))
    self._convs.append(conv(filters=num_filters/2, size=1, name=f'conv3_{i+1}'))
    self._convs.append(
        conv(filters=2, size=1, name=f'conv3_{i+2}', activation=None))
...
class PyramidFlowEstimator_3D(tf.keras.layers.Layer):
  def __init__(self, name: str, config: options.Options):
    super(PyramidFlowEstimator_3D, self).__init__(name=name)
    self._predictors = []
    for i in range(config.specialized_levels):
      self._predictors.append(FlowEstimator_3D(name=f'flow_predictor_3D{i}', num_convs=config.flow_convs[i], num_filters=config.flow_filters[i]))
    shared_predictor = FlowEstimator_3D(name='flow_predictor_shared_3D', num_convs=config.flow_convs[-1], num_filters=config.flow_filters[-1])
    for i in range(config.specialized_levels, config.pyramid_levels):
      self._predictors.append(shared_predictor)
